...
public:
     ...
     */
    ConjuntoPeliculas(int k = INCREMENTO);
    ...
    int numPeliculas() const;

    /**
     * Devuelve la posición en el vector de la película cuyo id coincide con busquedaid.
     * Si no se encuentra devuelve -1.
     */
    int busquedaPelicula(int busquedaid) const;

    /**
     * Busca en los títulos la primera película que contenga el substring nombrepelicula
     * y devuelve su id. Si no se encuentra, devuelve -1.
     */
    int busquedaPelicula(string nombrepelicula) const;

    ConjuntoPeliculas& operator=(const ConjuntoPeliculas& orig);
    ConjuntoPeliculas& operator+=(const Pelicula& newpelicula);
